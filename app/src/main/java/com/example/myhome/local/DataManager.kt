...
class DataManager2 @Inject constructor(
    private val sharedPreferences: MySharedPreference2?
) {
    fun saveToken( value: String) {
        sharedPreferences?.saveToken( value)
    }
    ...
    fun saveCurrentHouseId(houseId: String) {
        sharedPreferences?.saveCurrentHouseId(houseId)
    }
    fun getCurrentHouseId(): String? {
        return sharedPreferences?.getCurrentHouseId()
    }
